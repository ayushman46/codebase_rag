...
const AccountPage = () => {
  const {
    ...
    user,
    accountUsage: usage,
    accountUsageLoading: usageLoading,
    accountUsageError: usageError,
    fetchAccountUsage,
  } = useStore();
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      if (document.visibilityState !== 'hidden') void fetchAccountUsage({ silent: true });
    };
  ...
  return (
    <main className="narrow-shell page-section">
      ...
      <div className="mt-5 flex flex-col justify-between gap-8 sm:flex-row sm:items-end">
        ...
        <button onClick={signOut} className="outline-button inline-flex w-fit items-center gap-2 text-sm"> Sign out</button>
      </div>
      ...
      <section className="mt-8 rounded-[32px] border border-sand bg-pure-white p-7 sm:mt-10 sm:p-9" aria-labelledby="indexed-source-heading">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex items-center gap-2"><h2 id="indexed-source-heading" className="text-xl font-semibold tracking-tight text-ink-black">Indexed storage</h2></div>
        ...
        {usageLoading ? (
          ...
          <div className="mt-7">
            ...
            {usage.usage_estimated && <p className="mt-3 text-xs leading-relaxed text-warm-gray">Includes storage from an older index measured from its stored code sections. Re-index that repository once to record its exact source size.</p>}
          </div>
        ) : null}
      </section>
      ...
      <section className="mt-5 rounded-[32px] border border-sand bg-pure-white p-7 sm:p-9">
        ...
        <div className="mt-9 grid gap-4 border-t border-sand pt-7 sm:grid-cols-3">
          <div className="rounded-2xl bg-warm-canvas p-4"><p className="mt-4 text-caption font-semibold uppercase tracking-wider text-warm-gray">Email</p><p className="mt-1 truncate text-sm font-semibold text-charcoal">{user?.email}</p></div>
          <div className="rounded-2xl bg-warm-canvas p-4"><p className="mt-4 text-caption font-semibold uppercase tracking-wider text-warm-gray">Member since</p><p className="mt-1 text-sm font-semibold text-charcoal">{createdAt}</p></div>
          <div className="rounded-2xl bg-warm-canvas p-4"><p className="mt-4 text-caption font-semibold uppercase tracking-wider text-warm-gray">Indexed repos</p><p className="mt-1 text-sm font-semibold text-charcoal">{repos.length}</p></div>
        </div>
      </section>
    </main>
  );
